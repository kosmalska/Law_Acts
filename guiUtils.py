...
def mapInputs(self):
    ...
    if keywordsBool != False and keywordsText != "": 
        tmpKeyword = resolveKeywords(keywordsText)
        if tmpKeyword[0].isupper():
            inputMap["keywordName"] = resolveKeywords(keywordsText)
        else:
            inputMap["keyword"] = resolveKeywords(keywordsText)
